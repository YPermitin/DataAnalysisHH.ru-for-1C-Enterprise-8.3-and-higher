...
Процедура ПройтиАвторизацию(Команда)
	...
	ОткрытьФорму("ВнешнийОтчет.АнализДанныхHHRU.Форма.ЗапросКодаАвторизации",,ЭтаФорма, УникальныйИдентификатор,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИсточникВыбора.ИмяФормы = "ВнешнийОтчет.АнализДанныхHHRU.Форма.ЗапросКодаАвторизации" Тогда
...
Процедура ОбратнаяСвязь(Команда)
	...
	ЗапуститьПриложение("http://develplatform.ru/About-us/Contact-us?messageSubject=1С:Анализ%20данных%20HH.ru%20(отчет)");
...
Процедура ПосмотретьАнализНаКарте(Команда)
	...
	Если РезультатПроверкиАвторизации Тогда
		ЗапуститьПриложение("http://alm.develplatform.ru?forceToken="+Отчет.ТокенДоступа+"&forceQuery="+ТекстПоиска);
	Иначе
		ЗапуститьПриложение("http://alm.develplatform.ru");
		ПоказатьПредупреждение(,"Пожалуйтса, пройдите авторизацию!");
	КонецЕсли;
...
Процедура РезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если СтруктураРасшифровки.Поле = "Вакансии.ID" Тогда
		ЗапуститьПриложение("https://hh.ru/vacancy/"+СтруктураРасшифровки.Значение);
		СтандартнаяОбработка = Ложь;
	ИначеЕсли СтруктураРасшифровки.Поле = "Вакансии.Работодатель.РаботодательID" Тогда
		ЗапуститьПриложение("https://hh.ru/employer/"+СтруктураРасшифровки.Значение);
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
